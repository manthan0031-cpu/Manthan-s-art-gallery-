...
(function() {
    ...
    /* ==========================================
       CARD CLICK — whole card navigates to project
       ========================================== */
    function initCardLinks() {
        DOM.cards.forEach(card => {
            // Find the primary project link (card-link-btn or card-arrow)
            const primaryLink = card.querySelector('.card-link-btn') || card.querySelector('.card-arrow');
            if (!primaryLink) return;

            card.addEventListener('click', e => {
                // Let existing <a> elements handle their own clicks
    ...
    function initExpertiseCards() {
        ...
        mm.add({
            ...
            const spreads = isMobile ? [
                { xPercent: -110, yPercent: -110, rotateY: -8, rotateZ: -4 },
                { xPercent: 10,   yPercent: -110, rotateY: 8,  rotateZ: 4 },
                { xPercent: -110, yPercent: 10,   rotateY: -8, rotateZ: -4 },
                { xPercent: 10,   yPercent: 10,   rotateY: 8,  rotateZ: 4 }
            ] : [
            ...
            const zPeaks = isMobile ? [40, 60, 90, 120] : [70, 110, 170, 240];

            const tl = gsap.timeline({
                scrollTrigger: {
                    trigger: section,
                    start: 'top top',
                    end: isMobile ? '+=1000' : '+=1400',
                    scrub: 2.5,
                    pin: true,
                    anticipatePin: 1
                }
            ...
            cards.forEach((card, index) => {
                ...
                tl.to(card, {
                    ...
                    keyframes: [
                        ...
                        { z: index * 0.5, xPercent: target.xPercent, yPercent: target.yPercent, rotationY: 0, rotationZ: 0, duration: 0.2, ease: 'power2.out' }
                    ],
                    duration: 0.8
                }, staggerDelay);
            });
    ...
    function initFooterTransitions() {
        ...
        // Use hardcoded colors to avoid reading incorrect colors during loading/rendering states, fallback to dynamic base if specified
        const originalBgColor = document.body.dataset.bgBase || (isCaseStudy 
    ...
    function initAnimations() {
        ...
        // Cards staggered reveal (for all devices)
        ...
        // Process steps scroll animation (individual with drawing circles on mobile, staggered on desktop)
        const processStepsMM = gsap.matchMedia();
        processStepsMM.add({
            isMobile: "(max-width: 768px)",
            isDesktop: "(min-width: 769px)"
        }, (context) => {
            ...
            if (isMobile) {
                // Mobile: animate each process step and its circle individually
                document.querySelectorAll('.process-step').forEach((step) => {
                    const path = step.querySelector('.process-step-svg-circle path');
                    gsap.set(step, { y: 30, opacity: 0 });
                    if (path) {
                        gsap.set(path, { strokeDashoffset: 3000, opacity: 0 });
                    }
                    gsap.to(step, {
                        scrollTrigger: {
                            trigger: step,
                ...
                // Desktop: staggered reveal (CSS hover handles circle drawing)
    ...
    function init() {
        try {
            // Force correct html background immediately to avoid FOUC / flash of light background on case studies
            if (document.body.classList.contains('case-study-page')) {
                const bgBase = document.body.dataset.bgBase || (document.body.classList.contains('theme-light-book') ? '#F9F6F0' : '#121212');
                document.documentElement.style.backgroundColor = bgBase;
                ...
                initLoader(() => {
                    ...
                    requestAnimationFrame(() => {
                        setTimeout(() => { 
                            ...
                            // Initialize footer transitions AFTER the layout is refreshed
